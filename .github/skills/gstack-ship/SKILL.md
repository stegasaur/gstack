...
[ -n "$_ROOT" ] && [ -d "$_ROOT/.github/skills/gstack" ] && GSTACK_ROOT="$_ROOT/.github/skills/gstack"
GSTACK_BIN="$GSTACK_ROOT/bin"
GSTACK_BROWSE="$GSTACK_ROOT/browse/dist"
GSTACK_DESIGN="$GSTACK_ROOT/design/dist"
_UPD=$($GSTACK_BIN/gstack-update-check 2>/dev/null || .github/skills/gstack/bin/gstack-update-check 2>/dev/null || true)
...
_HAS_ROUTING="no"
if [ -f .github/copilot-instructions.md ] && grep -q "## Skill routing" .github/copilot-instructions.md 2>/dev/null; then
  _HAS_ROUTING="yes"
fi
...
_VENDORED="no"
if [ -d ".github/skills/gstack" ] && [ ! -L ".github/skills/gstack" ]; then
  if [ -f ".github/skills/gstack/VERSION" ] || [ -d ".github/skills/gstack/.git" ]; then
    _VENDORED="yes"
  fi
fi
...
Check if a .github/copilot-instructions.md file exists in the project root. If it does not exist, create it.

Use AskUserQuestion:

> gstack works best when your project's .github/copilot-instructions.md includes skill routing rules.

Options:
- A) Add routing rules to .github/copilot-instructions.md (recommended)
- B) No thanks, I'll invoke skills manually

If A: Append this section to the end of .github/copilot-instructions.md:
...
Then commit the change: `git add .github/copilot-instructions.md && git commit -m "chore: add gstack skill routing rules to .github/copilot-instructions.md"`
...
> This project has gstack vendored in `.github/skills/gstack/`. Vendoring is deprecated.
...
If A:
1. Run `git rm -r .github/skills/gstack/`
2. Run `echo '.github/skills/gstack/' >> .gitignore`
3. Run `$GSTACK_BIN/gstack-team-init required` (or `optional`)
4. Run `git add .claude/ .gitignore .github/copilot-instructions.md && git commit -m "chore: migrate gstack from vendored to team mode"`
...
if [ -f "$_GBRAIN_CONFIG" ] && command -v gbrain >/dev/null 2>&1; then
  ...
  if [ "$_GBRAIN_VERSION_OK" -gt 0 ] 2>/dev/null; then
    ...
    if [ -n "$_GBRAIN_PIN_PATH" ]; then
      ...
      echo "symbol-aware code lookup. See \"## GBrain Search Guidance\" in .github/copilot-instructions.md."
...
### B7. Update .github/copilot-instructions.md

First check: If .github/copilot-instructions.md already has a `## Testing` section → skip. Don't duplicate.
...
Only commit if there are changes. Stage all bootstrap files (config, test directory, TESTING.md, .github/copilot-instructions.md, .github/workflows/test.yml if created):
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `.github/skills/gstack/review/TODOS-format.md`).
...
Match against these patterns (from .github/copilot-instructions.md):
...
1. **Read .github/copilot-instructions.md** — look for a `## Testing` section with test command and framework name. If found, use that as the authoritative source.
2. **If .github/copilot-instructions.md has no testing section, auto-detect:**
...
Before proceeding, check .github/copilot-instructions.md for a `## Test Coverage` section with `Minimum:` and `Target:` fields. If found, use those percentages. Otherwise use defaults: Minimum = 60%, Target = 80%.
...
> You are classifying Greptile review comments for a /ship workflow. Read `.github/skills/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
> You are executing the /document-release workflow after a code push. Read the full skill file `${HOME}/.github/skills/gstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
> After completing the workflow, output a single JSON object on the LAST LINE of your response (no other text after it):
> `{"files_updated":["README.md",".github/copilot-instructions.md",...],"commit_sha":"abc1234","pushed":true,"documentation_section":"<markdown block for PR body's ## Documentation section>"}`
>
