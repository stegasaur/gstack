...
if [ ! -f ~/.gstack/projects/$SLUG/land-deploy-confirmed ]; then
  ...
  CURRENT_HASH=$(sed -n '/## Deploy Configuration/,/^## /p' .github/copilot-instructions.md 2>/dev/null | shasum -a 256 | cut -d' ' -f1)
...
```bash
# Check for persisted deploy config in .github/copilot-instructions.md
DEPLOY_CONFIG=$(grep -A 20 "## Deploy Configuration" .github/copilot-instructions.md 2>/dev/null || echo "NO_CONFIG")
echo "$DEPLOY_CONFIG"
...
If `PERSISTED_PLATFORM` and `PERSISTED_URL` were found in .github/copilot-instructions.md, use them directly
...
and any persisted config from .github/copilot-instructions.md.
...
1. **.github/copilot-instructions.md persisted config:** Check for a staging URL in the Deploy Configuration section:
```bash
grep -i "staging" .github/copilot-instructions.md 2>/dev/null | head -3
```
...
**If C:** **STOP.** "Running `/setup-deploy` will walk through your deploy platform, production URL, and health checks in detail. It saves everything to .github/copilot-instructions.md so I'll know exactly what to do next time. Run `/land-and-deploy` again when that's done."
...
Read .github/copilot-instructions.md to find the project's test command. If not specified, use `bun test`.
...
```bash
git diff --name-only $(gh pr view --json baseRefName -q .baseRefName 2>/dev/null || echo main)...HEAD -- README.md CHANGELOG.md ARCHITECTURE.md CONTRIBUTING.md .github/copilot-instructions.md VERSION
```
...
If staging was detected in Step 1.5c (or from .github/copilot-instructions.md deploy config), and the changes
...
If a deploy status command was configured in .github/copilot-instructions.md (e.g., `fly status --app myapp`), use it instead of or in addition to GitHub Actions polling.
...
If .github/copilot-instructions.md has a custom deploy status command in the "Custom deploy hooks" section, run that command and check its exit code.
