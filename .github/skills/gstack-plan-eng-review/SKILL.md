...
For LLM/prompt changes: check the "Prompt/LLM changes" file patterns listed in .github/copilot-instructions.md. If this plan touches ANY of those patterns, state which eval suites must be run, which cases should be added, and what baselines to compare against. Then use AskUserQuestion to confirm the eval scope with the user.
...
- Effort uses the AI-compression table from .github/copilot-instructions.md.
