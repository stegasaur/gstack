...
description: |
  ...
  guidance in .github/copilot-instructions.md. Wraps the gstack-gbrain-sync orchestrator with
...
guidance in .github/copilot-instructions.md so the coding agent knows when to prefer `gbrain`
search over Grep.
...
## Step 4: Refresh `## GBrain Search Guidance` block in .github/copilot-instructions.md
...
Then update .github/copilot-instructions.md based on capability state:
...
end of .github/copilot-instructions.md if it doesn't. NEVER duplicate. Block is machine-AGNOSTIC
...
the entire block at the end of .github/copilot-instructions.md.

**Atomic write:** write the new .github/copilot-instructions.md content to a tmp file alongside it
(e.g., `.github/copilot-instructions.md.sync-gbrain.tmp`) then `mv` to atomic-rename, so a crash
...
Do NOT crash if .github/copilot-instructions.md is missing or unwritable — log a warning and
continue.
...
gbrain status: GREEN
  ...
  .github/copilot-instructions.md ....... OK   ## GBrain Search Guidance present
...
check failed; .github/copilot-instructions.md guidance block REMOVED — run /setup-gbrain to repair`).
...
any state-file or .github/copilot-instructions.md mutation and exits with code 2 if another sync is
in flight. Stale locks (process died) auto-clear after 5 minutes.

## Cross-machine note

The `## GBrain Search Guidance` block is committed to the repo's .github/copilot-instructions.md
...
.github/copilot-instructions.md but no local gbrain, /sync-gbrain detects the mismatch via the
...
- **DONE** — all stages green, .github/copilot-instructions.md guidance block present, verdict GREEN.
